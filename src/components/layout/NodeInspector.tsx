...
export function NodeInspector({ node, onUpdate, isDarkMode = true }: NodeInspectorProps) {
  ...
  const nodeStyles = isDatabase
    ? {
        ...
        borderColor: isDarkMode ? 'rgba(139, 92, 246, 0.3)' : 'rgba(124, 58, 237, 0.25)',
    ...
    : {
        ...
        borderColor: isDarkMode ? 'rgba(59, 130, 246, 0.3)' : 'rgba(14, 165, 233, 0.25)',
        accentColor: isDarkMode ? '#60a5fa' : '#0ea5e9',
      }

  return (
    <div className="flex flex-col gap-4">
      {/* Service Node Card Preview - Matches Canvas Node */}
      <div
        className="rounded-2xl transition-all duration-200"
        style={{
          background: nodeStyles.background,
          border: `1.5px solid ${nodeStyles.borderColor}`,
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col gap-0.5">
              <h3 className="text-sm font-semibold leading-tight" style={{ color: isDarkMode ? '#ffffff' : '#000000' }}>{localName}</h3>
              <span 
